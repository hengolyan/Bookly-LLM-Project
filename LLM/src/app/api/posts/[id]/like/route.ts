...
import { likePostViaRest } from "@/lib/supabase-actions";
...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  try {
    ...
    try {
      ...
      logServerError("api.posts.like.prisma-fallback", prismaError);
      const liked = await likePostViaRest(user, params.id);
      return NextResponse.json({ status: "success", ...liked });
    }
  } catch (error) {
