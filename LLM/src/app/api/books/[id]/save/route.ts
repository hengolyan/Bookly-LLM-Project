...
import { saveBookViaRest } from "@/lib/supabase-actions";
...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  try {
    ...
    try {
      ...
      logServerError("api.books.save.prisma-fallback", prismaError);
      const saved = await saveBookViaRest(user, params.id);
